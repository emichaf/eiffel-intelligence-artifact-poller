     ...
     String GIT_DELETED
        ...
        stage ('GITHUB Checkout EI BackEnd Artifact SC') {
		    ...
		    dir ('sourcecode') {
                            ...
                            String[] author_parts = COMITTER_MAIL.split('@');
                            AUTHOR_NAME = author_parts[0].replace(".", " ");


                            //GIT_LONG_COMMIT =  sh(returnStdout: true, script: "git log --format='%H' -n 1").trim()
                            sh(returnStdout: true, script: "git log --shortstat -n 1 | (grep "file changed" || grep "files changed") | awk '{$GIT_FILES+=$1; $GIT_INSERTED+=$4; $GIT_DELETED+=$6} END'")
